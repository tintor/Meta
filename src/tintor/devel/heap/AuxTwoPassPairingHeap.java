...
public class AuxTwoPassPairingHeap<T extends Comparable<T>> extends AbstractQueue<T> {
	...
	public Node<T> offerX(final T e) {
		...
		aux = cons(a, aux);
		if (min == null || e.compareTo(min.element) < 0) min = a;
		size++;
		return a;
	}
	...
	public void decrease(final Node<T> a) {
		...
		aux = cons(a, aux);
		if (min == null || a.element.compareTo(min.element) < 0) min = a;
	}
	...
	public void merge(final AuxTwoPassPairingHeap<T> heap) {
		...
		if (min == null || heap.root.element.compareTo(min.element) < 0) min = heap.root;
	...
	@SuppressWarnings("null") private static <T extends Comparable<T>> Node<T> twopass(Node<T> listA) {
